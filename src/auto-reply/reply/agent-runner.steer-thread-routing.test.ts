...
describe("runReplyAgent steer mode thread routing", () => {
  ...
  it("allows fast steer when thread context not registered but both threads undefined", async () => {
    queueEmbeddedPiMessageMock.mockClear();
    enqueueFollowupRunMock.mockClear();
    getActiveRunThreadContextMock.mockClear();
    hasActiveRunThreadContextMock.mockClear();

    // Session is NOT registered (edge case: setActiveEmbeddedRun wasn't called)
    hasActiveRunThreadContextMock.mockReturnValue(false);
    getActiveRunThreadContextMock.mockReturnValue(undefined);
    // Steer succeeds
    queueEmbeddedPiMessageMock.mockReturnValue(true);
    ...
    // Message also has no thread ID - both undefined is always safe
    const followupRun = createFollowupRun({
      originatingThreadId: undefined,
    });
    ...
    // Fast steer SHOULD still be attempted (undefined/undefined is always safe)
    expect(queueEmbeddedPiMessageMock).toHaveBeenCalledWith("session", "hello");
  ...
  it("skips fast steer when thread ID types differ (string vs number)", async () => {
    queueEmbeddedPiMessageMock.mockClear();
    enqueueFollowupRunMock.mockClear();
    getActiveRunThreadContextMock.mockClear();
    hasActiveRunThreadContextMock.mockClear();
    ...
    // Incoming message has same value but as string - types differ, so skip fast steer
    // to avoid false positives from type coercion (e.g., float precision loss)
    const followupRun = createFollowupRun({
      originatingThreadId: "12345",
    });
    ...
    // Fast steer should NOT be attempted - type mismatch means fall back to queue
    expect(queueEmbeddedPiMessageMock).not.toHaveBeenCalled();
    expect(enqueueFollowupRunMock).toHaveBeenCalled();
  });
});
