...
- Audio transcription (if configured via `tools.audio.transcription`) runs before templating, replaces `Body` with an `[Audio]` block, and sets `{{Transcript}}`. Command parsing uses the transcript so slash commands still work.
- Video understanding (if configured via `tools.video.understanding`) runs before templating, replaces `Body` with a `[Video]` description, and preserves any caption text for command parsing.
- Only the first matching audio/video attachment is processed; remaining attachments are left untouched.
