    ...
    try {
      ...
      if (isPollEvent) {
        ...
        if (pollSummary) {
          ...
          const senderDisplayName = await getMemberDisplayName(
            roomId,
            senderId,
          );
      ...
      // Thread label for display context (agent can use thread-read tool for full context)
      const threadLabel = threadRootId
        ? `Matrix thread #${roomName ?? roomInfo.canonicalAlias ?? roomId}`
        : undefined;
      ...
      const threadContextNote = threadRootId
        ? `\n[Thread: use thread-read action with roomId="${roomId}" threadId="${threadRootId}" to fetch full thread context]`
        : "";
      ...
      if (didSendReply) {
        ...
        core.system.enqueueSystemEvent(
          `Matrix message from ${senderName}: ${previewText}`,
          {
            sessionKey: route.sessionKey,
